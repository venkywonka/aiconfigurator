...
def test_defaults_fill_in():
    cfg = SmartSearchConfig(
        ...
        workload={"isl": 4000, "osl": 1000, "request_rate": 25, "num_request_ratio": 1000},
    )
...
def test_planner_optimization_target_mapping():
    ...
    assert OptimizationTarget.THROUGHPUT_PER_USER.planner_optimization_target == "throughput"
...
def test_pareto_objectives_configurable():
    goal = OptimizationGoal(
        ...
        pareto_objectives=[OptimizationTarget.THROUGHPUT_PER_USER, OptimizationTarget.GOODPUT_PER_GPU],
        sla=SLATarget(ttft_ms=2000, itl_ms=30),  # goodput_per_gpu objective needs an SLA
    )
...
def test_pareto_objectives_only_with_pareto_target():
    with pytest.raises(ValidationError, match="only valid when target is 'pareto'"):
        OptimizationGoal(
            ...
            pareto_objectives=[OptimizationTarget.THROUGHPUT_PER_GPU, OptimizationTarget.THROUGHPUT_PER_USER],
        )
...
def test_pareto_goodput_objective_requires_sla():
    with pytest.raises(ValidationError, match="require an SLA"):
        OptimizationGoal(
            target=OptimizationTarget.PARETO,
            pareto_objectives=[OptimizationTarget.GOODPUT_PER_GPU, OptimizationTarget.THROUGHPUT_PER_USER],
        )
...
def test_pareto_objectives_must_be_distinct():
    with pytest.raises(ValidationError, match="distinct"):
        OptimizationGoal(
            target=OptimizationTarget.PARETO,
            pareto_objectives=[OptimizationTarget.THROUGHPUT_PER_GPU, OptimizationTarget.THROUGHPUT_PER_GPU],
        )
